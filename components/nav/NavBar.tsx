	...
	return (
		<>
			...
			<nav className={s.navbar}>
				...
				{showMenu ? (
					<button className={cn(s.item, s.close, 'nav', 'nav-hover')} onClick={() => setShowMenu(false)}>
						Close
					</button>
				) : (
			...
			{(!showMenu || !isDesktop) && <Cart localization={localization} />}
			<MenuDesktop
				menu={menu}
			...
			/>
			<MenuMobile menu={menu} localization={localization} showMenu={showMenu} />
		</>
	);
}
