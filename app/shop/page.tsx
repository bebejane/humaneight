...
export default async function Shop({ params }: CountryShopParams) {
	...
	const filteredProducts = allProducts
		?.filter((product) => !tag || tag === 'all' || product?.shopifyProduct?.tags?.split(',').includes(tag))
		.sort(sortByTag)
		.sort(sortByCollectionOrder);
	const totalVariantsCount =
		filteredProducts?.reduce((acc, product) => acc + getProductColorVariants(product as ProductRecord).length, 0) +
		filteredProducts?.length;
	const brandingInterval = 5;
	const brandings = generateRandomBranding<AllProductBrandingQuery['allProductBrandings'][0]>(
		Math.ceil(totalVariantsCount / brandingInterval),
		allProductBrandings
	);
	const brandingIndex = Array.from({ length: Math.ceil(totalVariantsCount / brandingInterval) }, (_, i) =>
		Math.floor(Math.random() * totalVariantsCount)
	);
	let brandingCount = 0;

	const tags = allProducts?.reduce(
		(acc, product) => {
	...
	return (
		<>
			<CollectionsFilter collectionId={collection?.id} allCollections={allCollections} tag={tag} tags={tags} />
...
function sortByCollectionOrder(
	a: AllProductByCollectionQuery['allProducts'][0],
	b: AllProductByCollectionQuery['allProducts'][0]
) {
