				...
				{product?.sections.map(({ id, productMedia, text }, i) => {
					...
					return (
						<div className={s.section} key={id}>
							{isDesktop ? (
								productMedia.map(({ variation, altText: alt }, idx) => {
									const mediaCount = mediaByVariation.length;
									const selectedVariation = variation.filter(
										(v) => v.color?.title?.toLowerCase() === color?.toLowerCase()
									);
									...
									return media.map(({ media: { id, responsiveImage } }, i) => (
										<figure key={id} className={cn(mediaCount > 1 && s.double)} onClick={() => setGalleryId(id)}>
											{responsiveImage && (
												<Image
													key={id}
													className={s.image}
