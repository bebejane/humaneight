...
export default function ProductVariantsForm({
	...
	return (
		<form
			...
			className={cn(s.form, className, isMobileHidden && s.hidden)}
			ref={formRef}
			style={formStyles}
		>
...
const availableVariants = (
	...
	return product.variants.edges
		.filter(({ node: { selectedOptions, availableForSale, quantityAvailable } }) => {
			return (
				availableForSale &&
				quantityAvailable &&
				selectedOptions.find((opt) => opt.name === name && opt.value === value) !== undefined
			);
		})
		.map(({ node }) => node);
};
