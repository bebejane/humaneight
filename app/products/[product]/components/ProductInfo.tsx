...
export default function ProductInfo({ product, shopifyProduct }: Props) {
	...
	return (
		<div className={s.details} aria-label='Product details'>
			<div className={s.info} ref={contentRef}>
				...
				{mainImage?.responsiveImage && (
					<figure className={s.mainImage}>
						<Image data={mainImage?.responsiveImage} className={s.image} imgClassName={s.picture} />
					</figure>
				)}
				...
				<section id={`${product.id}-description`} className={cn(s.description, 'light mid')} ref={descriptionRef}>
					<Content content={product.description} />
				</section>
			</div>
			<ProductVariantsForm product={product} shopifyProduct={shopifyProduct} mobile={false} />
		</div>
	);
}
