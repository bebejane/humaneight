...
export const upsertObject = async (object: ObjectMap, itemType: string, data: any) => {
	...
	let record = (
		await client.items.list({ version: 'latest', filter: { type: itemType, fields: { shopify_id: { eq: data.id } } } })
	)[0];
	const item = { ...data };

	if (object.dato_model === 'shopify_collection') {
		const products = await shopify.collection.products(data.id, { limit: 250 });
		const datoProducts = await Promise.all(
			products.map(({ id }) =>
				client.items.list({
					version: 'latest',
					filter: { type: 'shopify_product', fields: { shopify_id: { eq: id } } },
				})
			)
		);
	...
	if (object.dato_model === 'shopify_product') {
		...
		const datoCollections = await Promise.all(
			collections.map(({ id }) =>
				client.items.list({
					version: 'latest',
					filter: { type: 'shopify_collection', fields: { shopify_id: { eq: id } } },
				})
			)
		);
	...
	if (!record)
		record = await client.items.create({
			...
			...mapObject(object, data, item as ObjectType),
		});
	else record = await client.items.update(record.id, mapObject(object, data, item));

	//if (object.dato_model === 'shopify_product') await syncProductVariants(data)

	if (item.status === 'draft') await client.items.unpublish(record.id);
	else await client.items.publish(record.id);
};
...
export const syncDatoCMSObject = async (item: Item) => {
	...
	if (!item || !item.shopify_data) throw new Error('Invalid item');
...
export const syncProductVariants = async (data: IProduct) => {
	...
	const variantsToDelete = productVariants.filter(
		({ shopify_id }) => !variants.find((pv) => String(pv.id) === String(shopify_id))
	);
	const allDeletedVariants = variantsToDelete.map(({ id }) => client.items.destroy(id));

	const allNewVariants = variants
		?.filter(({ id }) => !productVariants.find((pv) => String(pv.shopify_id) === String(id)))
	...
	const optionsToDelete = productOptions.filter(
		({ shopify_id }) => !options.find((pv) => String(pv.id) === String(shopify_id))
	);
	const allDeletedOptions = optionsToDelete?.map(({ id }) => client.items.destroy(id));
	const allNewOptions = options
		?.filter(({ id }) => !productOptions.find((pv) => String(pv.shopify_id) === String(id)))
